...
\section{Estimation and Prediction for a New Station}
...
\end{equation}
\noindent where $\alpha$ is the step size, and $L_t$ is the likelihood given the observations at time $t$.  We approximate the likelihood based on Monte Carlo approach outlined in Section~\ref{mc_estimation}.  Given the current estimat, $\hat{\theta}$ we can then update the parameters for each day of data without having to refit the entire model.

Moreover, this provides a way to transition from the initial estimate for a new station, to the true estimate using a consistent and straightforward approach.
